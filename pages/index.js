...
export default function SalahApp() {
  ...
  // 3. Forced Hijri Date Helper (Forcing Rajab/Islamic months)
  const getHijri = (date) => {
    const f = new Intl.DateTimeFormat('ar-SA-u-ca-islamic-umalqura-nu-latn', {day:'numeric', month:'long', year:'numeric'});
    const parts = f.formatToParts(date).reduce((acc, p) => ({ ...acc, [p.type]: p.value }), {});
    return parts;
  };
    ...
    <div className="max-w-md mx-auto min-h-screen bg-[#001f3f] text-white pb-32 font-sans overflow-hidden">
      ...
      {installPrompt && (
        <div className="bg-emerald-500 p-4 sticky top-0 z-[100] cursor-pointer" onClick={handleInstall}>
          <span className="text-[#001f3f] font-black text-xs flex justify-center items-center gap-2 uppercase tracking-widest"><Download size={14}/> Install App</span>
        </div>
      )}
      ...
      {view === 'qibla' && (
        <div className="p-10 text-center">
          ...
          <div className="mt-14 space-y-4">
            ...
            <a href="https://qiblafinder.withgoogle.com/" target="_blank" className="flex items-center justify-center gap-2 text-white/40 text-[10px] font-bold uppercase hover:text-emerald-500 transition-colors">
